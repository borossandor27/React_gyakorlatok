...
> [!IMPORTANT] Az alkalmazásban csak egyetlen `BrowserRouter` lehet.
> [!IMPORTANT] Az egész alkalmazást be kell *burkolni* vele, hogy a router működjön.
...
> [!IMPORTANT]  A `Switch` komponens helyett kell használni a **React Router v6**-tól.
> [!NOTE]  A `Route` komponensek csak `Routes`-on belül használható.
...
> [!NOTE]  A `path="*"` használható alapértelmezett oldalként (404):

  ```jsx
      <Route path="*" element={<NotFound />} />
  ```

> [!NOTE]  Dinamikus útvonalak is használhatóak például:
